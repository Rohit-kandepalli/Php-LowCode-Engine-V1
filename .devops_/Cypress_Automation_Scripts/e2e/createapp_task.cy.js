describe('Creating New App', () => {
  it('should create a new app with valid details', () => {    
    cy.fixture('CreateAppDetails.json').then((credentials) => { 
      const { username, password, applicationName, description, hosting, addengineurlendpoint, type } = credentials
      ...
      cy.document().then((doc) => { 
        ...
        if (alertElement) {
          ...
          if(hosting == "Cloud Hosting"){              
            ...
            cy.get('input.form-control.form-control-sm[placeholder="SubDomain"]').clear().type(applicationName) // domain name after https://           
            ...
            if(type == 'page'){
              cy.get('.form-select.form-select-sm.w-auto.d-inline').eq(0).select(type);
              cy.get('.form-select.form-select-sm.w-auto.d-inline').eq(1).select('home');
